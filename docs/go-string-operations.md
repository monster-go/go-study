...
**为什么 `len("Go语言")` 是 8 而不是 4？**

`len` 统计的是 **UTF-8 编码后的字节数**，不是字符（rune）个数。逐字符拆开：

| 字符 | Unicode 码点 | UTF-8 字节（十六进制） | 字节数 |
|------|-------------|------------------------|--------|
| `G` | U+0047 | `47` | 1 |
| `o` | U+006F | `6F` | 1 |
| `语` | U+8BED | `E8 AF AD` | 3 |
...
ASCII 字符（如 `G`、`o`）在 UTF-8 里占 1 字节；常用汉字通常占 3 字节。因此 `len(s)` = 8，而 `len([]rune(s))` = 4。

同理 `len("你好")` = 6（两个汉字各 3 字节），不是 2。
...
**`string(65)` 为何得到 `"A"` 而不是 `"65"`？**

`string(n)` 的参数是 **Unicode 码点**（整数），不是十进制数字的文本：

```
65  →  Unicode U+0041  →  字符 'A'  →  string(65) = "A"
"65" →  文本「六十五」  →  须用 strconv.Itoa(65) 或 fmt.Sprintf("%d", 65)
```
...
`range` 遍历 string 时，第一个返回值是 **UTF-8 字节偏移**，不是「第几个字符」：
